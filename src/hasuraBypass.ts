...
// verifies it's Apptegy-shaped, and pulls all STEM teachers in a single pass.
...
// subject keywords used as ILIKE filters on group.title. these are the "teams"
// the district directory exposes — most are subject-named, so this narrows
// the query to STEM before we pull members.
const STEM_SUBJECT_KEYWORDS = [
  "math", "mathematics", "algebra", "geometry", "calculus", "statistics",
  "science", "biology", "chemistry", "physics", "environmental", "earth",
  "anatomy", "astronomy", "forensic",
  "computer", "computer science", "coding", "programming",
  "technology", "tech ed",
  "engineering", "robotics", "STEM", "STEAM",
];

// frequent false-positives from keyword substring matching on group.title
const NON_STEM_SUBJECT_EXCLUSIONS = [
  /\bphysical education\b/i,
  /\bpolitical science\b/i,
  /\bsocial science\b/i,
  /\blibrary science\b/i,
  /\bhealth science\b/i, // covers health-services people who sneak into "science"
];
...
export async function scrapeViaHasura(
  ...
  // 1. pull every STEM-keyword-matching group + its members + owning school
  const stemFilters = STEM_SUBJECT_KEYWORDS.map(
    (k) => `{ title: { _ilike: "%${k}%" } }`,
  ).join(",");
  const groupsQuery = `
    { groups(where: { _or: [${stemFilters}] }) {
        title
  ...
  //    in multiple STEM groups (math + STEM coach, etc.) — merge their subjects
  ...
  for (const g of groupsRes.groups) {
    if (isNonStemGroupTitle(g.title)) continue;
  ...
  const teachers: RawTeacherData[] = [...byKey.values()].map((t) => ({
    ...
    phone: t.phone,
    department: pickDeptLabel(t.subjects),
...
function isNonStemGroupTitle(title: string): boolean {
  return NON_STEM_SUBJECT_EXCLUSIONS.some((re) => re.test(title));
}
