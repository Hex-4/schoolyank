...
export async function run(
  ...
  if (judgments && judgments.length === candidates.length) {
    ...
    teachers = [];
    const rejectedByJudge: string[] = [];
    for (let i = 0; i < candidates.length; i++) {
      const j = byIdx.get(i);
      if (!j || !j.isStem) {
        const c = candidates[i]!;
        rejectedByJudge.push(`${c.firstName} ${c.lastName} (${c.role})`);
        continue;
      }
    ...
    if (rejectedByJudge.length > 0) {
      debug("ORCH", `LLM judge rejected ${rejectedByJudge.length} non-STEM`, rejectedByJudge);
    }
    milestone(
  ...
  if (teachers.length === 0) {
    ...
    milestone(
      `⚠ pipeline returned 0 teachers — the scraper could not reach a staff directory on this site. common causes: (1) CMS with no public staff listing (e.g. Apptegy-CMS districts), (2) cross-domain redirect the scraper didn't follow, (3) LLM judge rejected all extracted candidates.`,
      "warn",
    );
  ...
  if (config.enableLinkedin && teachers.length > 0) {
    ...
    // also drop teachers whose linkedin role reveals a non-STEM subject (e.g.
    // scraper had dept="Science" but linkedin says "History Teacher").
    const NON_STEM_SIGNALS = /\b(history|english|language arts|spanish|french|german|latin|chinese|japanese|korean|esl|ell|art\b|music|drama|theater|theatre|dance|choir|band|orchestra|social studies|psychology|sociology|philosophy|religion|theology|world languages?|foreign language|humanities|creative writing|journalism|speech|debate|pe\b|physical education|health|athletic|counselor|librarian|media specialist)\b/i;
    const preLinkedinCount = teachers.length;
    teachers = teachers.filter((t) => {
      if (!t.sources.includes("linkedin")) return true;
      if (NON_STEM_SIGNALS.test(t.role) && !isStemRole(t.role, t.department)) {
        debug("ORCH", `post-linkedin STEM drop: ${t.firstName} ${t.lastName} — role "${t.role}" is non-STEM`);
        return false;
      }
      return true;
    });
    if (teachers.length < preLinkedinCount) {
      const dropped = preLinkedinCount - teachers.length;
      milestone(`dropped ${dropped} teacher${dropped === 1 ? "" : "s"} after LinkedIn revealed non-STEM role`, "warn");
    }
    for (const t of teachers) {
